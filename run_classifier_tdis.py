...
class prune_function:
    def __init__(self,args):
        ...
        if args.svd_weight_dir is None:
            ...
            if args.bert_model == 'bert-base-uncased':
                ...
                output_config_file = '/home/yujwang/maoyh/bert_config_tiny.json'
            '''else:
        ...
        if args.local_rank != -1:
            ...
            model_t = torch.nn.DataParallel(model)
    ...
    def eval_pt(self,sr,wr,target_prune_rate):
        ...
        for layer_now in range(layer_num):
            ...
            r = 1-target_prune_rate[layer_now*4+id[i]]**(1.*wr/split)#1-target_r^{sr_now/split} for each layer
            for i in range(12):
                do_sparse(svd_ch[i], r, None, model)

        prune_type = ['svd'] * 48
        prune_rate = [target_prune_rate[i]**(1.*sr/split) for i in range(48)]
    ...
    def eval_after_train(self,prune_type,target_prune_rate):
        ...
        for epoch_i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                    ...
                    if sr_now+wr_now<step//intv and sr_now+wr_now<split: # sr_now, wr_now are increased here
                        if mode=='direct':
                            if sr_now<sr_target and sr_now<=wr_now:
                                ...
                                wr_now+=1
                        elif self.eval_pt(sr_now+1,wr_now,target_prune_rate)>self.eval_pt(sr_now,wr_now+1,target_prune_rate):
                            sr_now+=1#*=rpr
                        else:
                            wr_now+=1#*=rpr
                else:
                ...
                logits = model(input_ids, segment_ids, input_mask, p_type=['vanilla'], p_rate=[1.]*48)
                with torch.no_grad():
                ...
                if True:#distill
                    loss_dst=MSELoss()
                    loss+=loss_dst(logits.view(-1),logits_t.view(-1))
                #distill
                ...
                '''if step>0:# weight pruning is done here
                    ...
                    for layer_now in range(layer_num):
                        svd_ch = [model.bert.encoder.layer[layer_now].attention.self.qmat1[:, :model.bert.encoder.layer[layer_now].attention.self.to_dim],
                                  model.bert.encoder.layer[layer_now].attention.self.qmat2[:model.bert.encoder.layer[layer_now].attention.self.to_dim, :],
                                  model.bert.encoder.layer[layer_now].attention.self.kmat1[:, :model.bert.encoder.layer[layer_now].attention.self.to_dim],
                                  model.bert.encoder.layer[layer_now].attention.self.kmat2[:model.bert.encoder.layer[layer_now].attention.self.to_dim, :],
                                  model.bert.encoder.layer[layer_now].attention.self.vmat1[:, :model.bert.encoder.layer[layer_now].attention.self.to_dim],
                                  model.bert.encoder.layer[layer_now].attention.self.vmat2[:model.bert.encoder.layer[layer_now].attention.self.to_dim, :],
                                  model.bert.encoder.layer[layer_now].attention.output.dmat1[:, :model.bert.encoder.layer[layer_now].attention.output.to_dim],
                                  model.bert.encoder.layer[layer_now].attention.output.dmat2[:model.bert.encoder.layer[layer_now].attention.output.to_dim, :],
                                  model.bert.encoder.layer[layer_now].intermediate.dmat1[:, :model.bert.encoder.layer[layer_now].intermediate.to_dim],
                                  model.bert.encoder.layer[layer_now].intermediate.dmat2[:model.bert.encoder.layer[layer_now].intermediate.to_dim, :],
                                  model.bert.encoder.layer[layer_now].output.dmat1[:, :model.bert.encoder.layer[layer_now].output.to_dim],
                                  model.bert.encoder.layer[layer_now].output.dmat2[:model.bert.encoder.layer[layer_now].output.to_dim, :]]
                        id=[0,0,0,0,0,0,1,1,2,2,3,3]
                        for i in range(12):
...
def main():
    ...
    parser.add_argument("--split",
                        default=64,
                        type=int,
                        help="Total rounds of iterative pruning.")
    ...
    task_step={'mnli':10000,'sst-2':2100,'qnli':3000,'qqp':10000}
    ...
    for tst in to_test_sr:
        ...
        sr_target,wr_target=tst,split-tst
        if True:
            ...
            prune_type = ['svd']*48
            for i in range(48):
                prune_rate.append((sp['pr' + str(i)]-0.3)/0.4*0.1*args.lw+1.)
            prune_rate = balance(prune_rate, args.target_r)
