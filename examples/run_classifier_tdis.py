        ...
        for epoch_i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                ...
                if output_mode == "classification":
                    ...
                    loss = 0*loss_fct(logits.view(-1, num_labels), label_ids.view(-1))
                elif output_mode == "regression":
                    loss_fct = MSELoss()
                    loss = loss_fct(logits.view(-1), label_ids.view(-1))
                somx=torch.nn.Softmax()
                if True:#distill
                    loss_dst=BCELoss()
                    loss+=1*loss_dst(somx(logits.view(-1)),somx(logits_t.view(-1)))#-sum(somx(logits.view(-1))*somx(logits_t.view(-1)).log())
                #distill
                loss_dst=MSELoss()
            ...
            if epoch_i % 1 == 0:
                ...
                for input_ids, input_mask, segment_ids, label_ids in tqdm(self.eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss = model(input_ids, segment_ids, input_mask, label_ids, p_type=['vanilla']*48,p_rate=[1.]*48)
                        logits = model(input_ids, segment_ids, input_mask, p_type=['vanilla']*48, p_rate=[1.]*48)
...
def main():
    ...
    print(func.eval_after_train(['vanilla']*48,[1.]*48))
